...
const EmailLink = () => {
  ...
  const [message, updateMessage] = useState(messagesCopy[idx]);
  const [char, updateChar] = useState(messagesCopy[idx].length); // points to current char
  ...
  useInterval(
    () => {
      ...
      if (char - hold >= messagesCopy[idx].length) {
        newIdx += 1;
        newChar = 0;
      }

      if (newIdx === messagesCopy.length) {
        newIdx = 0; // Restart from the first message
      }

      updateMessage(messagesCopy[newIdx].slice(0, newChar));
      updateIter(newIdx);
...
const ContactLinks = (props) => {
  ...
  const contactLinks = props.items.map((item) => {
    return (
      <li key={item.name} className="contacts-item">
        <a
          className={"contacts-link" + (position ? " fade-in-left-" + i-- : "")}
          title={"Link to Naveen's " + item.name}
          href={item.link}
          target="social"
        >
          {item.icon}
      ...
      <section id="contacts" className="contacts-section">
        <div className="contacts-container container to-slide up">
          <div className="contacts-text">
            <h3>Get in touch </h3>
            <p className="mail-contacts">
              If you want to get in touch with me for any reason, may it be
              about work or just to say hi, send me a mail at{" "}
              <a className="mail-link" href="mailto:gnpaone@gmail.com">
                <EmailLink />
              </a>{" "}
          ...
          <div className="contacts-mobile">
            <p>
              Alternatively, if you wish to follow me or to contact me in other
              ways, you can also find me on{" "}
            </p>
